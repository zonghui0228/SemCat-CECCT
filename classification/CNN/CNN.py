...
def convolution_net(data, input_dim, class_dim, emb_dim, hid_dim):
    ...
    conv_5 = fluid.nets.sequence_conv_pool(
        ...
        act="tanh",
        pool_type="sqrt")
    prediction = fluid.layers.fc(input=[conv_3, conv_4, conv_5], size=class_dim, act="softmax")
    return prediction
...
if __name__ == "__main__":
    ...
    params_dirname = "eligibilitycriteria_conv.inference.model"  # 保存模型。参数

    train = False   # 控制是否进行训练
    predict = True # 进行预测
    evaluate = False # 在测试集上进行评估
